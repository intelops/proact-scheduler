...
class ScanConfigs(Base):
    ...
    pyroscope_enabled = Column(Boolean, default=False)
...
class ExecutionResponseNew(BaseModel):
    execution_id: str
    job_id: str
    vulnerable_packages_count: int
    vulnerablitites_count: int
...
class ScheduleDetailsResponseNew(BaseModel):
    schedule_id: str
    schedule_name: str
    total_scan_images_count: int
    total_vulnerable_images_count: int
    total_vulnerablities_count: int
    executions: list[ExecutionResponseNew]
